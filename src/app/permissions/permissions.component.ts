...
export class PermissionsComponent implements OnInit {
  ...
  statusBadge(status: string): string {
    ...
    return `<span class="inline-flex items-center gap-1.5 rounded-xl px-2.5 py-1 text-xs font-semibold ${active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}">
      <span class="h-1.5 w-1.5 rounded-full ${active ? 'bg-emerald-700' : 'bg-slate-400'}"></span>${status}
    </span>`;
  }
